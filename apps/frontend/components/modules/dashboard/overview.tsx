...
interface WorkOrderStats {
  ...
  pending_service_forms: number;
  whatsapp_sent_24h: number;
  whatsapp_read_rate_pct: number;
...
function ActivityIcon({ eventType }: { eventType: string }) {
  switch (eventType) {
    case "whatsapp_read":          return <MessageSquare className="h-3 w-3 text-blue-500" />;
    case "whatsapp_delivered":     return <CheckCheck className="h-3 w-3 text-emerald-500" />;
    case "whatsapp_failed":        return <AlertTriangle className="h-3 w-3 text-red-500" />;
    case "service_form_submitted": return <FileText className="h-3 w-3 text-violet-500" />;
    case "work_order_created":     return <ClipboardList className="h-3 w-3 text-amber-500" />;
    default:                       return <TrendingUp className="h-3 w-3 text-slate-400" />;
  }
}
...
export function DashboardOverview() {
  ...
  const [activities, setActivities] = useState<ErpNotification[]>([]);
  const [lastRefresh, setLastRefresh] = useState(new Date());

  const fetchWoData = useCallback(async () => {
    try {
      const [stats, acts] = await Promise.all([
        apiGet<WorkOrderStats>("/work-orders/stats").catch(() => null),
        apiGet<ErpNotification[]>("/notifications?days=1&limit=10").catch(() => [] as ErpNotification[]),
      ]);
      if (stats) setWoStats(stats as WorkOrderStats);
      setActivities(Array.isArray(acts) ? (acts as ErpNotification[]) : []);
  ...
  useEffect(() => {
    fetchWoData();
    const interval = setInterval(fetchWoData, 30000);
      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Tüm operasyon süreçlerinizi, saha durumlarını ve mağaza aktivitelerini tek ekrandan takip edin.
          </p>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-slate-900">Saha Operasyonlari</h2>
            </div>
            <button
              onClick={fetchWoData}
          ...
          <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 xl:grid-cols-6">
            <StatCard icon={ClipboardList} label="Acik Is Emirleri" value={woStats.open_count} sub="Aktif gorevler" iconClass="bg-blue-50 text-blue-600" />
            <StatCard icon={Plus} label="Bugun Olusturulan" value={woStats.today_created} sub={new Date().toLocaleDateString("tr-TR", { weekday: "long" })} iconClass="bg-emerald-50 text-emerald-600" />
            <StatCard icon={FileText} label="Bekleyen Servis Formu" value={woStats.pending_service_forms} sub="Yuklenmesi gerekiyor" iconClass="bg-amber-50 text-amber-600" />
            <StatCard icon={MessageSquare} label="WhatsApp (24s)" value={woStats.whatsapp_sent_24h} sub="Gonderilen mesaj" iconClass="bg-violet-50 text-violet-600" />
            <StatCard icon={TrendingUp} label="Okunma Orani" value={`%${woStats.whatsapp_read_rate_pct}`} sub="Son 7 gun" iconClass="bg-sky-50 text-sky-600" />
            <StatCard
              icon={AlertTriangle}
              label="Basarisiz (24s)"
              value={woStats.whatsapp_failed_24h}
              sub="Gonderilemeyen"
              iconClass={woStats.whatsapp_failed_24h > 0 ? "bg-red-50 text-red-500" : "bg-slate-100 text-slate-400"}
            />
          </div>
        </div>
      )}
      ...
      {activities.length > 0 && (
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100">
            ...
            <span className="text-[11px] font-semibold text-slate-400 bg-slate-100 rounded-full px-2 py-0.5">son 24 saat</span>
          </div>
            ...
            {activities.slice(0, 8).map((act) => (
              <div key={act.id} className="flex items-start gap-3 px-5 py-3">
                <div className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-slate-100">
                  <ActivityIcon eventType={act.event_type} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-slate-800">{act.title}</p>
                  {act.description && <p className="text-[11px] text-slate-400 mt-0.5 truncate">{act.description}</p>}
                </div>
                <p className="text-[10px] text-slate-300 shrink-0 mt-0.5">{relativeTime(act.created_at)}</p>
              </div>
            ))}
          </div>
        </div>
