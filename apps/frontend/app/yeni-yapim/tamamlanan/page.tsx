...
export default function TamamlananYeniYapimPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const yeniYapimJobs = (Array.isArray(jobs) ? jobs : []).filter(j => j.work_type === "yeni_yapim");

        const paymentsByProject: Record<string, ProgressPayment[]> = {};
        (Array.isArray(payments) ? payments : [])
          .filter(p => p.payment_type === "yeni_yapim")
          .forEach(p => {
        ...
        console.warn("Tamamlanan yeni yapım yüklenemedi:", e);
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-lg font-bold text-slate-900">Tamamlanan Yeni Yapım İşleri</h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Yeni yapım süreci tamamlanmış iş kalemleri — fatura ve hakkediş takibi
          </p>
        </div>
        <div className="relative w-full max-w-xs">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 pl-9 pr-4 py-2 text-sm focus:border-emerald-500 focus:outline-none"
          />
        </div>
      </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-4 rounded-2xl border border-dashed border-slate-200">
          ...
          <div className="text-center">
            <p className="text-sm font-semibold text-slate-500">
              {query ? "Sonuç bulunamadı" : "Tamamlanan yeni yapım yok"}
            </p>
            <p className="text-xs text-slate-400 mt-1">
              Yeni yapım süreci tamamlananlar burada görünür.
            </p>
          </div>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-slate-100 bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide hidden sm:table-cell">İş Kalemi</th>
                  ...
                  <tr key={`${r.job.project_id}-${r.job.process_id}`} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/yeni-yapim/surecleri/${r.job.process_id}?p=${r.job.project_id}`}
                          className="inline-flex items-center gap-1 text-[11px] text-emerald-600 border border-emerald-100 rounded-lg px-2 py-1 hover:bg-emerald-50">
                          <Receipt className="h-3 w-3" /> Klasör
                        </Link>
                        <Link href={`/projects/${r.job.project_id}`}
