...
function isBakim(payment: ProgressPayment, project: Project): boolean {
  if (payment.payment_type === "bakim") return true;
  if (["ara", "final"].includes(payment.payment_type)) {
    return (project.scope_codes ?? []).includes("bakim");
  }
  return false;
}
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900">Bakım Hakkedişleri</h1>
            <p className="text-xs text-slate-500">Bakım & Onarım kapsamındaki hakkediş kayıtları</p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3 rounded-2xl border border-dashed border-slate-200">
          ...
          <p className="text-sm font-semibold text-slate-500">
            {query ? "Arama sonucu bulunamadı." : "Henüz bakım hakkedişi girilmemiş."}
          </p>
