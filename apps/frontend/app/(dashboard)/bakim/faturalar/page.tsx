...
import { CheckCircle2, Clock, FolderOpen, Receipt, Search } from "lucide-react";
...
type InvoiceRecord = {
  ...
  amount?: number; currency: string; file_name?: string;
...
export default function BakimFaturalarPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-slate-900">Bakım Faturaları</h1>
          <p className="text-xs text-slate-500">Bakım mağazalarına ait aylık fatura takibi</p>
        </div>
      </div>
              ...
              {filtered.map(p => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs font-semibold text-slate-800">{fmtTRY(rec?.amount, rec?.currency)}</td>
