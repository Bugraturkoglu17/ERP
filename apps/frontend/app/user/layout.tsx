...
function UserSidebar({
  ...
  const inner = (
    <div className="flex h-full flex-col bg-teal-950 text-teal-100">
      <div className="flex h-16 items-center justify-between px-4 border-b border-teal-900">
        <div className="flex items-center gap-2.5">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-teal-500 text-white text-xs font-bold">
            K
          </div>
          <span className="text-sm font-semibold text-white">Kullanıcı Paneli</span>
        </div>
            ...
            <Link
              ...
              className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors ${
                active
                  ...
                  : "text-teal-300 hover:bg-teal-900 hover:text-teal-100"
              }`}
            >
      ...
      <DevAccountSwitcher theme="teal" />
    </div>
  );

  return (
    <>
      <aside className="hidden lg:block w-60 shrink-0 border-r border-teal-900">{inner}</aside>
      {mobileOpen && (
...
export default function UserLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <RoleGuard allowedRoles={["USER"]} spinnerBg="bg-white">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-6">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-medium text-slate-500">{fullName}</span>
