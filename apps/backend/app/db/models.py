...
class ProcurementStatus(str, Enum):
    """Satın alma talebi / PO durumu."""
    DRAFT            = "draft"             # Taslak
    PENDING_APPROVAL = "pending_approval"  # Onay bekliyor
    APPROVED         = "approved"          # Onaylandı
    REJECTED         = "rejected"          # Reddedildi
    ORDERED          = "ordered"           # Siparişe çevrildi
...
class Supplier(SQLModel, table=True):
    """
    Tedarikçi firma — boru, vana, sismik askı vb. malzeme satıcıları.
    Satın alma siparişleri buraya bağlanır.
    """
...
class PurchaseRequest(SQLModel, table=True):
    """
    Malzeme Talep Fişi — saha mühendisi veya depo sorumlusu tarafından açılır.
    Admin onayladıktan sonra PO'ya dönüşür.
    """
...
class PurchaseOrder(SQLModel, table=True):
    """
    Satın Alma Siparişi (PO) — onaylı talepten oluşur.
    Teslim alındığında otomatik Stok IN hareketi tetiklenir.
    """
...
class PurchaseOrderItem(SQLModel, table=True):
    """PO satır kalemi — malzeme, miktar ve birim fiyat."""
...
class FieldReportActivityType(str, Enum):
    """Aktivite türü — saha günlük raporlarında iş kaydı için."""
    INSTALLATION   = "installation"   # Montaj
    TESTING        = "testing"        # Test ve devreye alma
    INSPECTION     = "inspection"     # Keşif / denetim
    PROCUREMENT    = "procurement"    # Malzeme temin
    DOCUMENTATION  = "documentation"  # Çizim / döküman işi
    COORDINATION   = "coordination"   # Koordinasyon toplantısı
    OTHER          = "other"          # Diğer


class FieldReportItem(SQLModel, table=True):
    """
    Günlük saha raporunun aktivite satırı.
    Her rapor birden fazla aktivite içerebilir.
    Örnek: '3. Kat sismik askı montajı tamamlandı — 4 saat'
    """
    ...
    location:        Optional[str]          = Field(default=None, max_length=200)  # Kat / alan
